...
class PackageCreator
{
	...
	constructor()
	{
		...
		catch {return console.log('Could not load config file');}
	...
	{
		if (Object.keys(conf).length == 0 || conf.enabled == false) return this.betterLog(depth, 'Ignoring configuration of', chalk.grey(name));
		this.betterLog(depth, 'Beginning configuration of', chalk.blue(name));
		let env = {...conf};
	...
	{
		if (actions.enabled == false) return this.betterLog(depth, 'Ignoring actions for', chalk.grey(name), 'because disabled');
	...
	{
		if (ext.enabled == false) return this.betterLog(depth + 1, 'Ignoring extension', chalk.grey(name), 'because disabled');
		if (ext.actions)
		{
			this.loadConfiguration(ext, name, depth + 1);
			return ;
		}
		if (!ext.extension) return this.betterLog(depth + 1, 'Ignoring extension', chalk.grey(name), 'because configuration is missing');
